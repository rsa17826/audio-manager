...
class AudioManager(tk.Tk):
  ...
  def _on_rclick(self, mp3: Path):
    item = self._items.get(mp3)
    if not item:
      return
    # Clear active selection if this was the active item
    if self._active and self._active["mp3"] == mp3:
      self._active = None
      self._active_var.set("— no file selected —")
    # Remove tile
    item["tile"].destroy()
    self._items.pop(mp3, None)
    self._reflow()
    # Trash all three source files
